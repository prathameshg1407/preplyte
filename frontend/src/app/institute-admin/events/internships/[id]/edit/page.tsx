// src/app/institute-admin/events/internships/[id]/edit/page.tsx
...
import { OpportunityStatus } from '@/types/event.types';
...
export default function EditInternshipPage() {
  ...
  useEffect(() => {
    const fetchInternship = async () => {
      try {
        setLoading(true);
        const internship = await opportunityService.getInternship(params.id as string);
        ...
        setFormData({
          ...
          internshipType: internship.internshipType as any,
          durationValue: internship.durationValue,
          durationType: internship.durationType as any,
          workMode: internship.workMode as any,
          stipendMin: internship.stipendMin || 0,
          stipendMax: internship.stipendMax || 0,
          vacancies: internship.vacancies,
          isPpo: internship.isPpo || false,
          location: internship.location,
          applicationDeadline: formattedDate,
          isResumeRequired: internship.isResumeRequired,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      await opportunityService.updateInternship(params.id as string, {
        ...formData,
      ...
      toast({ title: 'Error', description: 'Failed to update internship.', variant: 'destructive' });
    } finally {
      setSubmitting(false);
    }
  };
