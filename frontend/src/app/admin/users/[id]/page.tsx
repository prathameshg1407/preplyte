...
import { usePlatformAdminViewStudentDashboard } from '@/lib/hooks/admin/use-view-student';
...
export default function PlatformAdminStudentProfilePage() {
    const params = useParams();
    const router = useRouter();
    const studentId = params.id as string;

    const { data, isLoading, error, refetch } = usePlatformAdminViewStudentDashboard(studentId);
    ...
    // Handle optional dashboard data
    const stats = dashboard?.stats || {};
    const recentTests = dashboard?.recentTests || [];
    const upcomingTests = dashboard?.upcomingTests || [];
    const lms = dashboard?.lms || {
        stats: {},
        enrollments: { all: [], inProgress: [], completed: [] },
        recentActivity: []
    };
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Detailed performance and academic records (Admin View)</p>
                    </div>
                </div>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-xl font-bold text-primary">{stats.overallScore || 0}%</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-xs text-muted-foreground">LMS Points</p>
                                    <p className="text-xl font-bold text-yellow-600">{lms.stats?.totalPointsEarned || 0}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-xs text-muted-foreground">Learning Hours</p>
                                    <p className="text-xl font-bold">{lms.stats?.totalLearningHours || 0}h</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-xs text-muted-foreground">Certificates</p>
                                    <p className="text-xl font-bold">{lms.stats?.certificatesEarned || 0}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Tabs defaultValue="overview" className="w-full">
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="flex justify-between items-center text-sm">
                                            ...
                                            <span className="font-bold">{lms.stats?.completedCourses || 0}/{lms.stats?.totalEnrollments || 0}</span>
                                        </div>
                                        ...
                                        <div className="flex justify-between items-center text-sm">
                                            ...
                                            <span className="font-bold">{lms.stats?.moduleTestsPassed || 0}</span>
                                        </div>
                        ...
                        <TabsContent value="lms" className="mt-6 space-y-6">
                            <div className="grid gap-4 md:grid-cols-3">
                                <StatsMiniCard title="Active Courses" value={lms.stats?.inProgressCourses || 0} icon={Play} />
                                <StatsMiniCard title="LMS Points" value={lms.stats?.totalPointsEarned || 0} icon={Trophy} />
                                <StatsMiniCard title="Average Progress" value={`${lms.stats?.averageProgress || 0}%`} icon={TrendingUp} />
                            </div>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className="space-y-4">
                                        ...
                                        {(!lms.enrollments?.all || lms.enrollments.all.length === 0) && (
                        ...
                        <TabsContent value="practice" className="mt-6 space-y-6">
                            <div className="grid gap-4 md:grid-cols-4">
                                <StatsMiniCard title="Aptitude" value={stats.testsCompleted || 0} icon={Brain} />
                                <StatsMiniCard title="Coding" value={stats.problemsSolved || 0} icon={Code} />
                                <StatsMiniCard title="AI Mock" value={stats.interviewsCompleted || 0} icon={MessageSquare} />
                                <StatsMiniCard title="Overall" value={`${stats.overallScore || 0}%`} icon={TrendingUp} />
                            </div>
