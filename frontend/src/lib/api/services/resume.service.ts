...
export const resumeTemplateService = {
  getAll: async (filters?: TemplateFilters): Promise<ResumeTemplate[]> => {
    ...
    const response = await axiosInstance.get<ApiResponse<ResumeTemplate[]>>(
  ...
  getCategories: async (): Promise<TemplateCategoryCount[]> => {
    const response = await axiosInstance.get<ApiResponse<TemplateCategoryCount[]>>(
...
export const resumeService = {
  create: async (data: CreateResumeRequest): Promise<Resume> => {
    const response = await axiosInstance.post<ApiResponse<Resume>>(BASE_URL, data);
    return response.data.data;
  },
  ...
  getAll: async (filters?: ResumeFilters): Promise<ResumesResponse> => {
    ...
    const response = await axiosInstance.get<ApiResponse<ResumesResponse>>(
  ...
  getById: async (resumeId: string): Promise<Resume> => {
    const response = await axiosInstance.get<ApiResponse<Resume>>(
      `${BASE_URL}/${resumeId}`
    );
    return response.data.data;
  },
  ...
  update: async (resumeId: string, data: UpdateResumeRequest): Promise<Resume> => {
    const response = await axiosInstance.patch<ApiResponse<Resume>>(
      `${BASE_URL}/${resumeId}`,
      data
    );
    return response.data.data;
  ...
  delete: async (resumeId: string): Promise<void> => {
    await axiosInstance.delete(`${BASE_URL}/${resumeId}`);
  },

  duplicate: async (resumeId: string, newTitle?: string): Promise<Resume> => {
    const response = await axiosInstance.post<ApiResponse<Resume>>(
  ...
  getVersions: async (resumeId: string): Promise<ResumeVersion[]> => {
    const response = await axiosInstance.get<ApiResponse<ResumeVersion[]>>(
