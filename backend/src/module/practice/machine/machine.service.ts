...
const createPagination = (page: number, limit: number, totalItems: number) => ({
  currentPage: page,
...
class MachineService {
  ...
  async createSession(userId: string, dto: CreateSessionDto) {
    ...
    if (selectedQuestions.length < dto.numberOfQuestions) {
      logger.warn(`Insufficient questions. Found: ${selectedQuestions.length}, Requested: ${dto.numberOfQuestions}`);
    }
  ...
  async getQuestion(userId: string, sessionId: string, questionId: string) {
    ...
    if (currentIndex === -1) throw new NotFoundError('Question in this session');

    const sq = session.sessionQuestions[currentIndex];
    const totalTestCases = await prisma.testCase.count({ where: { questionId } });
  ...
  async runCode(userId: string, sessionId: string, questionId: string, dto: RunCodeDto): Promise<CodeExecutionResult> {
  ...
  async submitCode(userId: string, sessionId: string, questionId: string, dto: SubmitCodeDto): Promise<SubmissionResult> {
    const { sessionQuestion } = await this.validateSessionForExecution(userId, sessionId, questionId);
    ...
    try {
      const tokens = await judge0.createBatchSubmission(
        testCases.map((tc) => ({
  ...
  async getSubmissionHistory(userId: string, sessionId: string, questionId: string, page: number, limit: number) {
    const session = await prisma.machinePracticeSession.findFirst({
      where: { id: sessionId, userId },
      include: {
        sessionQuestions: {
    ...
    const [totalItems, submissions] = await Promise.all([
      prisma.submission.count({ where: { sessionQuestionId: sessionQuestion.id } }),
  ...
  private async ensureNoActiveSession(userId: string): Promise<void> {
  ...
  private async fetchQuestions(dto: CreateSessionDto) {
    const where: Prisma.MachineQuestionWhereInput = {
      isActive: true,
      difficulty: dto.difficulty,
    };
    ...
    return prisma.machineQuestion.findMany({ where });
  }

  private buildSessionWhereClause(userId: string, filters: SessionListFilters): Prisma.MachinePracticeSessionWhereInput {
  ...
  private async validateSessionForExecution(userId: string, sessionId: string, questionId: string) {
    const session = await prisma.machinePracticeSession.findFirst({
      where: { id: sessionId, userId },
      include: {
        sessionQuestions: {
    ...
    if (!session) throw new NotFoundError('Session');
    if (session.completedAt) throw new SessionAlreadyCompletedError(session.completedAt);
    if (new Date() > session.expiresAt) throw new SessionExpiredError(session.expiresAt);
  ...
  private async runWithCustomInput(sessionId: string, questionId: string, dto: RunCodeDto): Promise<CodeExecutionResult> {
    try {
  ...
  private async processSubmissionResults(
    submissionId: string,
    sessionId: string,
    questionId: string,
    sessionQuestion: { id: string; isSolved: boolean },
    testCases: Array<{ type: string; input: string; expectedOutput: string }>,
    results: any[]
  ...
  private analyzeResults(results: any[], totalTestCases: number) {
    ...
    if (testCasesPassed < totalTestCases && firstFailedIndex >= 0) {
      status = judge0.mapStatusToSubmissionStatus(results[firstFailedIndex].status.id) as SubmissionStatus;
    }

    return { testCasesPassed, totalTime, maxMemory, firstFailedIndex, status };
  }

  private async markSubmissionError(submissionId: string, error: unknown): Promise<void> {
    await prisma.submission.update({
      where: { id: submissionId },
      data: {
  ...
  private waitForJudge0(): Promise<void> {
    return new Promise((resolve) => setTimeout(resolve, 2000));
  }

  private mapResultStatus(passed: boolean, statusId: number): TestCaseResult['status'] {
  ...
  private calculateExecutionStats(results: TestCaseResult[]) {
  ...
  private formatQuestionResult(sq: any) {
    const bestSubmission = sq.submissions.find((s: any) => s.status === 'ACCEPTED') || sq.submissions[0];
  ...
  private evaluatePerformance(solved: number, total: number, difficulty: DifficultyLevel): PerformanceEvaluation {
    ...
    if (solved < total) suggestions.push('Review the unsolved problems and learn from their solutions');
    if (difficulty !== 'EASY' && solvedPercentage < 50) suggestions.push('Consider practicing with easier problems first');
    if (suggestions.length === 0) suggestions.push('Try challenging yourself with harder problems');
