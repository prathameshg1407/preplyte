...
import { EligibilityCheck, EligibilityCheckResult } from '../shared/mockdrive.shared-types';
...
export function checkEligibility(
  ...
  if (criteria.minCgpa !== null) {
    ...
    if (!passed) failedCriteria.push(`Minimum CGPA: ${criteria.minCgpa}`);
  }
  ...
  if (criteria.maxCgpa !== null) {
    ...
    if (!passed) failedCriteria.push(`Maximum CGPA: ${criteria.maxCgpa}`);
  }
  ...
  if (criteria.minMarks10 !== null) {
    ...
    if (!passed) failedCriteria.push(`Minimum 10th Marks: ${criteria.minMarks10}%`);
  }
  ...
  if (criteria.minMarks12 !== null) {
    ...
    if (!passed) failedCriteria.push(`Minimum 12th Marks: ${criteria.minMarks12}%`);
  }
  ...
  if (criteria.requiredSkills && criteria.requiredSkills.length > 0) {
    const hasSkill = profile.skills.some((skill) =>
      criteria.requiredSkills.some(
        (required) => skill.toLowerCase().includes(required.toLowerCase())
      )
    );
    checks.push({
...
export function formatEligibilityResult(result: EligibilityCheckResult): string {
