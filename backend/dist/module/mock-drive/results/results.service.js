...
class ResultsService {
    ...
    async getResultOverview(userId, driveId) {
        ...
        if (!attempt) {
            throw new errors_1.NotFoundError('No attempt found for this mock drive');
        }
        if (attempt.status !== 'COMPLETED' && attempt.status !== 'TIMED_OUT') {
            throw new errors_1.BadRequestError('Results are not available yet. Complete the mock drive first.');
        }
    ...
    async getDetailedReport(userId, driveId) {
        ...
        if (!attempt) {
            throw new errors_1.NotFoundError('Attempt');
        }
        // Generate module reports
        const moduleReports = await Promise.all(attempt.moduleAttempts.map(async (ma) => {
    ...
    generateModuleAnalysis(moduleType, moduleData) {
        if (!moduleData)
            return null;
        switch (moduleType) {
            case 'APTITUDE':
                return this.generateAptitudeAnalysis(moduleData);
            case 'MACHINE_CODING':
                return this.generateMachineAnalysis(moduleData);
            case 'AI_INTERVIEW':
                return this.generateInterviewAnalysis(moduleData);
            default:
                return null;
        }
    }
    // src/module/mock-drive/results/results.service.ts
    generateAptitudeAnalysis(data) {
    ...
    generateInterviewAnalysis(data) {
    ...
    generateModuleFeedback(moduleType, analysis, percentage) {
        if (!analysis)
            return 'No detailed analysis available.';
        const performanceLevel = percentage >= 80
            ? 'excellent'
            : percentage >= 60
                ? 'good'
                : percentage >= 40
    ...
    generateModuleRecommendations(moduleType, analysis) {
        if (!analysis)
            return [];
    ...
    generateOverallInsights(moduleReports) {
        const strengths = [];
        const weaknesses = [];
        const recommendations = [];
    ...
    generateOverallFeedback(overview, _moduleReports) {
    ...
    async getComparisonStats(driveId, userId, batchId) {
        // Get batch stats
