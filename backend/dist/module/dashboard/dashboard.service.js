...
class DashboardService {
    ...
    async getStudentRecentTests(userId) {
        const [aptitudeSessions, machineSessions, interviewSessions] = await Promise.all([
            db_1.prisma.aptitudePracticeSession.findMany({
    ...
    async getStudentUpcomingDrives(userId) {
        const now = new Date();
        ...
        const registrations = await db_1.prisma.mockDriveRegistration.findMany({
            ...
            take: dashboard_constants_1.DASHBOARD_LIMITS.UPCOMING_TESTS,
        });
    ...
    // =================================================
    async getInstituteAdminDashboard(userId, instituteId) {
        logger_1.logger.debug('[DashboardService] Fetching institute admin dashboard', {
            userId,
            instituteId,
        });
        ...
        if (!user || user.instituteId !== instituteId) {
            throw new errors_1.ForbiddenError('Access denied to this institute');
        }
    ...
    async getInstituteStats(instituteId) {
        const { startOfThisMonth, startOfLastMonth, endOfLastMonth } = (0, dashboard_constants_1.getDateRanges)();
        // Get all drives for this institute
        const drives = await db_1.prisma.mockDrive.findMany({
            where: { instituteId },
            select: {
                id: true,
                status: true,
        ...
        const drivesThisMonth = drives.filter((d) => d.createdAt >= startOfThisMonth).length;
        const activeDrives = drives.filter((d) => d.status === client_1.MockDriveStatus.IN_PROGRESS).length;
        const upcomingDrives = drives.filter((d) => d.driveStartDate &&
            d.driveStartDate > now &&
            d.status !== client_1.MockDriveStatus.CANCELLED).length;
        // Get registration stats
        const driveIds = drives.map((d) => d.id);
        const [totalRegistrations, registrationsThisMonth] = await Promise.all([
            db_1.prisma.mockDriveRegistration.count({
        ...
        const [currentMonthAttempts, lastMonthAttempts] = await Promise.all([
            db_1.prisma.mockDriveAttempt.findMany({
                where: {
                    mockDriveId: { in: driveIds },
    ...
    async getInstituteRecentDrives(instituteId) {
        const drives = await db_1.prisma.mockDrive.findMany({
            where: { instituteId },
            orderBy: { createdAt: 'desc' },
            take: dashboard_constants_1.DASHBOARD_LIMITS.RECENT_DRIVES,
            select: {
                id: true,
                title: true,
                status: true,
    ...
    async getInstituteTopPerformers(instituteId) {
        ...
        const leaderboardEntries = await db_1.prisma.mockDriveLeaderboard.findMany({
            where: {
                mockDriveId: { in: driveIds },
                batchId: null, // Overall leaderboard
            },
            orderBy: { percentageScore: 'desc' },
            take: dashboard_constants_1.DASHBOARD_LIMITS.TOP_PERFORMERS * 2, // Get more to aggregate
            select: {
                userId: true,
                studentName: true,
                studentId: true,
        ...
        const performers = Array.from(userScores.values())
            .map((user) => ({
            userId: user.userId,
            ...
            .slice(0, dashboard_constants_1.DASHBOARD_LIMITS.TOP_PERFORMERS);
        return performers;
    }
    // =================================================
    ...
    async getPlatformAdminDashboard() {
        logger_1.logger.debug('[DashboardService] Fetching platform admin dashboard');
    ...
    async getPlatformOverview() {
        const [totalInstitutes, activeInstitutes, userCounts,] = await Promise.all([
            db_1.prisma.institute.count(),
            db_1.prisma.institute.count({ where: { isActive: true } }),
    ...
    async getPlatformSessionStats() {
        const [aptitude, machine, interview] = await Promise.all([
            ...
            db_1.prisma.aiInterviewSession.aggregate({
                _count: true,
            }),
        ]);
        const [completedAptitude, completedMachine, completedInterview] = await Promise.all([
            db_1.prisma.aptitudePracticeSession.count({
                where: { completedAt: { not: null } },
            }),
            db_1.prisma.machinePracticeSession.count({
                where: { completedAt: { not: null } },
            }),
            db_1.prisma.aiInterviewSession.count({
    ...
    async calculateAptitudeAverage() {
        const sessions = await db_1.prisma.aptitudePracticeSession.findMany({
            where: {
        ...
        if (sessions.length === 0)
            return 0;
        const percentages = sessions.map((s) => (s.totalScore / s.numberOfQuestions) * 100);
        return Math.round((percentages.reduce((a, b) => a + b, 0) / percentages.length) * 10) / 10;
    }
    async calculateMachineAverage() {
        const sessions = await db_1.prisma.machinePracticeSession.findMany({
            where: {
                completedAt: { not: null },
        ...
        if (sessions.length === 0)
            return 0;
        const percentages = sessions.map((s) => (s.totalSolved / s.numberOfQuestions) * 100);
        return Math.round((percentages.reduce((a, b) => a + b, 0) / percentages.length) * 10) / 10;
    }
    async calculateInterviewAverage() {
        const feedbacks = await db_1.prisma.aiInterviewFeedback.findMany({
            select: { overallScore: true },
            take: 1000,
            orderBy: { createdAt: 'desc' },
        });
    ...
    async getPlatformTrends() {
        const { last30Days } = (0, dashboard_constants_1.getDateRanges)();
        // Get user registrations trend
        const userRegistrations = await db_1.prisma.user.groupBy({
            by: ['createdAt'],
            where: {
        ...
        const aptitudeTrend = await db_1.prisma.aptitudePracticeSession.groupBy({
            by: ['startedAt'],
            where: {
        ...
        const machineTrend = await db_1.prisma.machinePracticeSession.groupBy({
            by: ['startedAt'],
            where: {
                startedAt: { gte: last30Days },
            },
        ...
        const interviewTrend = await db_1.prisma.aiInterviewSession.groupBy({
            by: ['createdAt'],
            where: {
                createdAt: { gte: last30Days },
            },
        ...
        const userTrend = this.aggregateTrendByDate(userRegistrations.map((r) => ({
            date: r.createdAt,
            count: r._count,
        })));
    ...
    async getPlatformRecentInstitutes() {
