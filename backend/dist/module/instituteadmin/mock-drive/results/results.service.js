...
class ResultsService {
    ...
    // List Results
    // ==========================================
    async listResults(mockDriveId, instituteId, query) {
        await this.verifyAccess(mockDriveId, instituteId);
        const { page = DEFAULT_PAGE, limit = DEFAULT_LIMIT, batchId, status, search, sortBy = 'rank', sortOrder = 'asc', } = query;
        ...
        const [attempts, total] = await Promise.all([
            ...
            db_1.prisma.mockDriveAttempt.count({ where }),
        ]);
        const data = attempts.map((a) => ({
            ...
            departmentId: a.user.profile?.departmentId ?? null,
    ...
    // Get Detailed Result
    // ==========================================
    async getDetailedResult(mockDriveId, attemptId, instituteId) {
        await this.verifyAccess(mockDriveId, instituteId);
        const attempt = await db_1.prisma.mockDriveAttempt.findUnique({
            where: { id: attemptId },
            include: {
                user: { include: { profile: true } },
                batch: { select: { id: true, name: true } },
        ...
        if (!attempt || attempt.mockDriveId !== mockDriveId) {
            throw new results_types_1.ResultNotFoundError(attemptId);
        }
        ...
        return {
            ...
            student: {
                ...
                departmentId: attempt.user.profile?.departmentId ?? null,
            },
    ...
    // Get Statistics
    // ==========================================
    async getStatistics(mockDriveId, instituteId, batchId) {
        await this.verifyAccess(mockDriveId, instituteId);
        const baseWhere = {
            mockDriveId,
            ...(batchId && { batchId }),
        };
        const completedWhere = { ...baseWhere, status: client_1.MockDriveAttemptStatus.COMPLETED };
        const [total, completed, passed, scores] = await Promise.all([
            db_1.prisma.mockDriveAttempt.count({ where: baseWhere }),
            db_1.prisma.mockDriveAttempt.count({ where: completedWhere }),
            db_1.prisma.mockDriveAttempt.count({ where: { ...completedWhere, isPassed: true } }),
    ...
    // Calculate Rankings
    // ==========================================
    async calculateRankings(mockDriveId, instituteId, batchId) {
        await this.verifyAccess(mockDriveId, instituteId);
        ...
        await db_1.prisma.$transaction(rankings.map((r) => db_1.prisma.mockDriveAttempt.update({
            where: { id: r.attemptId },
            data: { rank: r.rank },
        })));
        logger_1.logger.info('Rankings calculated', { mockDriveId, batchId, count: rankings.length });
        return rankings;
    }
    // ==========================================
    // Export Results
    // ==========================================
    async exportResults(mockDriveId, instituteId, options) {
        await this.verifyAccess(mockDriveId, instituteId);
        const [mockDrive, attempts] = await Promise.all([
            db_1.prisma.mockDrive.findUnique({
        ...
        if (attempts.length === 0) {
            throw new results_types_1.ResultsError('NO_DATA', 'No results to export', 400);
        }
        const rows = attempts.map((a) => ({
            ...
            Email: a.user.email,
            Department: a.user.profile?.departmentId ?? 'N/A',
    ...
    // ==========================================
    async generateReport(mockDriveId, attemptId, instituteId) {
        await this.verifyAccess(mockDriveId, instituteId);
        const attempt = await db_1.prisma.mockDriveAttempt.findUnique({
            where: { id: attemptId },
            include: {
        ...
        if (attempt.status !== client_1.MockDriveAttemptStatus.COMPLETED) {
            throw new results_types_1.ResultsError('NOT_COMPLETED', 'Cannot generate report for incomplete attempt', 400);
        }
        if (attempt.report) {
        ...
        await db_1.prisma.mockDriveReport.create({
            data: {
                attemptId,
        ...
        logger_1.logger.info('Report generated', { attemptId });
    }
    // ==========================================
    // Generate All Reports (Bulk)
    // ==========================================
    async generateAllReports(mockDriveId, instituteId) {
        await this.verifyAccess(mockDriveId, instituteId);
        const attempts = await db_1.prisma.mockDriveAttempt.findMany({
            where: {
                mockDriveId,
        ...
        for (const attempt of attempts) {
            try {
                ...
                logger_1.logger.error('Failed to generate report', { attemptId: attempt.id, error });
                failed++;
            }
        }
        const skipped = await db_1.prisma.mockDriveReport.count({
    ...
    async verifyAccess(mockDriveId, instituteId) {
        ...
        if (!mockDrive) {
            throw new results_types_1.MockDriveNotFoundError(mockDriveId);
        }
        if (mockDrive.instituteId !== instituteId) {
            throw new results_types_1.AccessDeniedError();
        }
    }
